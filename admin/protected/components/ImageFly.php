...
class ImageFly
{
     ...
     * @throws Exception
     */
    public function get($model, $attribute, $width = self::MEDIUM, $height = self::MEDIUM, $mode='proportional') {

        $path = $this->getThumbName($model, $width, $height, $mode);
        ...
        $savePath = $this->pathRoot . $model->imagePathThumb . $mode  . $width . "_" . $height . DIRECTORY_SEPARATOR;
        
        if(!file_exists($filePath . $imageName)) return '';
        
        $c = filemtime($filePath . $imageName)."_".$imageName;
        
        if (!file_exists($this->pathRoot . $path . $c)) 
        {
            if(!is_dir($this->pathRoot . $model->imagePathThumb . $mode  . $width . "_" . $height))
            {            
                $listfolderName = explode("/", $model->imagePathThumb);
                $listfolderName[] =  $mode  . $width . "_" . $height;
                self::validCreateFoldersFiles($listfolderName, true, $this->file);
            }

            $image = $this->resizeAndSave($filePath.$imageName, $savePath, $c, $model, $width, $height, $mode);
        }
        else
        {
    ...
    private function validImage($filename, $imageName)
    {
        ...
        if(count($check)==2 || count($check)==3){
     ...
     * @return string
     */
    private function resizeAndSave($imagePath, $savePath, $imageName, $model, $width, $height, $mode='proportional') {
        
        $canvas = canvas::Instance();
        $canvas->carrega($imagePath)
               ->redimensiona($width, $height, $mode)
               ->grava($savePath.$imageName);
        
        return $this->webRoot.$this->getThumbName($model, $width, $height, $mode).$imageName;
    }
     ...
     * @return string
     */
    private function getThumbName($model, $width, $height, $mode) {
        return $model->imagePathThumb . $mode . $width . '_' . $height . DIRECTORY_SEPARATOR;
    }
     ...
     * @throws Exception
     */
    public function getPath($model, $attribute, $prefix='') {
        
        if ($model->$attribute instanceof CUploadedFile) {
            return "images" . DIRECTORY_SEPARATOR . $model->imagePath . md5($prefix.$model->$attribute->getName()) . "." . $model->$attribute->getExtensionName();
        }
