...
class CourseController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Course']))
		{
			...
			if($university_id > 0){
				$_POST['Course']['university_id'] = $university_id;
			}

			$model->attributes=$_POST['Course'];
			if($model->save()){

				$join_model = new JoinGradeClassCourse();

				$join_model->grade = $_POST['Course']['grade'];
				$join_model->grade = $_POST['Course']['class'];
				$join_model->grade = $model->course_id;
		...
		// get subject
		$subject_data = Subject::model()->findAll();
		$subjects= array();
		foreach ($subject_data as $data) { 
			$subjects[$data['subject_id']] = $data['subject_name'];
		}
